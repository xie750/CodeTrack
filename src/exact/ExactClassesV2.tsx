...
export function ExactClassesV2(props: Props) {
  ...
  return <div className="exact-course-page class-v2-page">
    ...
    <Drawer rootClassName="class-detail-drawer" open={detailOpen} onClose={() => setDetailOpen(false)} placement="right" size={440} closeIcon={<X size={17} />} title={<Space size={8}><Title level={3}>{selected?.name}</Title><Tag color="green">主讲班级</Tag></Space>}>
    ...
    <Drawer rootClassName="class-join-status-drawer" open={joinOpen} onClose={() => setJoinOpen(false)} placement="right" size={820} closeIcon={<X size={18} />} title={<div className="class-join-drawer-title"><span><UserCheck size={20} /></span><div><Title level={3}>学生加入状态</Title><Text type="secondary">{joinStatus?.class_name || selected?.name}</Text></div></div>}>
