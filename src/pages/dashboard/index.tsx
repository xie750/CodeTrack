...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (todoParam) {
      const t: Record<string, string> = { '科研项目审核': '/research/projects', '合规审查': '/research/compliance' }
  ...
  // ===== 渲染 =====
  const todoItems = [
    { label: '科研项目审核', desc: '新提交的立项申请等待审核', count: projectTodo, path: '/research/projects?status=待审核' },
    { label: '合规审查处置', desc: 'AI 初检疑似违规需人工复核', count: complianceTodo, path: '/research/compliance?status=待处理' },
  ]
    ...
    <div>
      ...
      <Row gutter={12}>
        <Col xs={24} sm={12} md={4}>
          <MiniStat icon={<GraduationCap size={22} />} label="学生总数" value={studentCount.toLocaleString()} color={colors.primary} trend={5} trendLabel="较上月" onClick={() => navigate('/users/students')} />
        </Col>
        <Col xs={24} sm={12} md={4}>
          <MiniStat icon={<Users size={22} />} label="教师总数" value={teacherCount.toLocaleString()} color={colors.purple} trend={3} trendLabel="较上月" onClick={() => navigate('/users/teachers')} />
        </Col>
        <Col xs={24} sm={12} md={4}>
          <MiniStat icon={<School size={22} />} label="课程总数" value={courseCount.toLocaleString()} color={colors.info} trend={2} trendLabel="较上月" onClick={() => navigate('/users/classes')} />
        </Col>
        <Col xs={24} sm={12} md={4}>
          <MiniStat icon={<FlaskConical size={22} />} label="科研项目" value={projects.length} color={colors.warning} trend={8} trendLabel="较上周" onClick={() => navigate('/research/projects')} />
        </Col>
        <Col xs={24} sm={12} md={4}>
          <MiniStat icon={<Zap size={22} />} label="模型成功率" value={`${avgSuccess}%`} color={colors.purple} trend={0.3} trendLabel="较昨日" onClick={() => navigate('/ai/monitor')} />
        </Col>
        <Col xs={24} sm={12} md={4}>
          <MiniStat icon={<Server size={22} />} label="模型在线" value={`${modelOnline}/${modelTotal}`} color={colors.success} trend={3} trendLabel="课程垂类" onClick={() => navigate('/ai/route')} />
        </Col>
      </Row>
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 16 }}>
                  ...
                  {pendingAlertList.length > 0 ? (
                    pendingAlertList.map((a) => (
                      ...
                      >
                        ...
                        <Button size="small" type="primary" onClick={() => navigate('/ai/monitor')} style={{ flexShrink: 0 }}>去处理</Button>
                      </div>
                    ))
                  ) : (
                    <div style={{ padding: '32px 0', textAlign: 'center' }}>
        ...
        <div style={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Card
            ...
            extra={<a onClick={() => navigate('/system/notices')} style={{ fontSize: 12 }}>详情</a>}
          >
          ...
          <Card
            ...
            style={{ borderRadius: 12, flex: '1 1 0%', minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
            styles={{ body: { padding: '6px 14px 10px', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' } }}
            extra={<a onClick={() => navigate('/system/logs')} style={{ fontSize: 12 }}>详情</a>}
          >
            <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
            {recentLogs.map((l) => (
