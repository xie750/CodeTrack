...
from backend.app.services.openmaic_generator import generate_openmaic_classroom
...
AI_CLASSROOM_PROMPT_VERSION = "openmaic_native_v2"
MAX_SOURCE_COUNT = 5
...
async def _generate_model_ai_classroom_payload(
    *,
    ...
    payload = await generate_openmaic_classroom({
        "course": {"id": course.id, "name": course.name},
        "message": message,
        "knowledge_point": knowledge_point,
        "profile": _profile_focus(profile),
        "sources": [
            {**_source_payload(source), "content": _trim(source.content or source.summary, 6000)}
            for source in sources
        ],
    })
    ...
    payload["material"] = {
        ...
        "parsedBy": "openmaic_native",
    }
    return payload["stage"]["name"], payload["stage"]["description"], payload, len(payload["scenes"])
...
async def _ai_classroom_content_node(db: Session, course: Course, state: AIClassroomResourceState) -> AIClassroomResourceState:
    ...
    try:
        ...
        detail = exc.detail if isinstance(exc.detail, dict) else {}
        record_step(
            db, state["run"], step_name="model_classroom_generation", step_order=3,
            status="FAILED", output_summary={"fallback": False, "error": detail},
        )
        finish_run(
            db, state["run"], status="FAILED",
            error_code=detail.get("code", "CLASSROOM_GENERATION_FAILED"),
            error_message=detail.get("message", "课堂生成失败"),
        )
        # Preserve the failed run and user request; never create a READY template resource.
        db.commit()
        raise
    ...
    record_step(
        db, state["run"], step_name="model_classroom_generation", step_order=3,
        output_summary={
            "used_model": True, "pipeline": "openmaic_native",
            "usage": render_payload.get("metadata", {}).get("generation_usage", {}),
        },
    )
    citations = [_citation(source) for source in sources]
    state["title"] = title
    ...
    *,
    ...
    finish_run(
        ...
        output={
            ...
            "generation_pipeline": "openmaic_native",
        },
        model_provider="OPENAI_COMPATIBLE",
        model_name=get_settings().model_name,
        token_prompt=state["render_payload"].get("metadata", {}).get("generation_usage", {}).get("prompt_tokens"),
        token_completion=state["render_payload"].get("metadata", {}).get("generation_usage", {}).get("completion_tokens"),
        prompt_version=AI_CLASSROOM_PROMPT_VERSION,
    )
    return _serialize_resource(resource)
