...
export function ExactAnalytics({ courseId, classId, classes, notify }: FlowProps & { classes: ApiClass[] }) {
  ...
  return <div className="exact-course-page exact-analytics">
    ...
    {view === '班级总览' && <><div className="analytics-charts"><section><strong>成绩分布</strong><SimpleBarPanel data={data.score_distribution} labelKey="range" color="#43b81a" /></section><section><strong>高频错误</strong><SimpleBarPanel data={data.errors} labelKey="name" color="#e4a42d" /></section></div><div className="knowledge-analysis"><strong>知识点掌握度</strong>{data.knowledge.map((item: any) => <div key={item.id}><span>{item.name}</span><Progress percent={item.mastery} strokeColor={item.mastery < 60 ? '#d95f59' : item.mastery < 75 ? '#e4a42d' : '#43b81a'} /><b>{item.mastery}%</b><Tag color={item.mastery < 60 ? 'red' : item.mastery < 75 ? 'gold' : 'green'}>{item.mastery < 60 ? '薄弱' : item.mastery < 75 ? '需关注' : '稳定'}</Tag></div>)}</div></>}
